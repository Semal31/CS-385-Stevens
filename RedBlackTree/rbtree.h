    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x != nullptr) {
            ...
            if (key == x->key()) {
                ...
                error << key;
                throw tree_exception("Attempt to insert duplicate key '" + error.str() + "'.");
            }
            if (key < x->key())
                x = x->left;
            else
...
private:
    ...
    int diameter(Node<K, V> *node) const {
        ...
        if (node == nullptr)
            return 0;
        int sum_of_height = 3 + height(node->left) + height(node->right);
        int max_diameter = std::max(diameter(node->left), diameter(node->right));
        return std::max(sum_of_height, max_diameter);
    }
